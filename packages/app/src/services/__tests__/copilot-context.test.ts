...
function mockRepos(seed: {
  caregivers: core.Caregiver[];
  courses: core.LearningCourse[];
  selfCaregiver?: core.Caregiver;
}): void {
...
function caregiver(overrides: Partial<core.Caregiver> = {}): core.Caregiver {
  return {
    id: 'caregiver-1',
    agencyId: 'agency-1',
    firstName: 'Maria',
...
function course(overrides: Partial<core.LearningCourse> = {}): core.LearningCourse {
  return {
    id: 'course-1',
    agencyId: null,
    code: 'HIPAA-2026',
...
describe('buildCopilotContext', () => {
  ...
  it('caps caregivers at 50 for admin role', async () => {
    const many = Array.from({ length: 80 }, (_, i) =>
      caregiver({ id: `cg-${i}`, firstName: `First${i}`, lastName: `Last${i}` }),
    );
