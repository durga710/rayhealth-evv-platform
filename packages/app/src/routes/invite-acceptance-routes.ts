...
async function fetchInvite(db: Knex, token: string): Promise<InviteRow | undefined> {
  const row = (await db('staff_invites').where({ token }).first()) as
    | InviteRow
    | undefined
  return row
}

async function fetchAgencyName(db: Knex, agencyId: string): Promise<string> {
  try {
    const row = (await db('agencies').where({ id: agencyId }).first('name')) as
      | { name?: string }
      | undefined
...
router.get('/:token', async (req: Request, res: Response) => {
  try {
    ...
    let effectiveStatus: PublicInviteInfo['status']
