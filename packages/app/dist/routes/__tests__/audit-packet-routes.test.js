...
function installMocks(options = {}) {
    const getVisitByIdForAgency = vi.fn().mockResolvedValue(options.visit === undefined ? baseVisit() : options.visit);
    vi.spyOn(core, 'EvvRepository').mockImplementation(() => ({ getVisitByIdForAgency }));
    const findById = vi.fn().mockResolvedValue(options.caregiver === undefined ? { firstName: 'Jane', lastName: 'Caregiver' } : options.caregiver);
    vi.spyOn(core, 'CaregiverRepository').mockImplementation(() => ({ findById }));
    const getClientNameForAgency = vi.fn().mockResolvedValue(options.client === undefined ? { id: clientId, firstName: 'John', lastName: 'Client' } : options.client);
    const getClientGeofence = vi.fn().mockResolvedValue(options.clientGeofence === undefined
        ? { latitude: PA_CAPITOL.lat, longitude: PA_CAPITOL.lng, geofenceRadiusM: 150 }
        : options.clientGeofence);
    vi.spyOn(core, 'ClientRepository').mockImplementation(() => ({ getClientNameForAgency, getClientGeofence }));
    const getAssignmentScheduleForAgency = vi.fn().mockResolvedValue(options.schedule === undefined
        ? { scheduledStartTime: '2026-06-01T14:00:00.000Z', scheduledEndTime: '2026-06-01T16:00:00.000Z' }
        : options.schedule);
    vi.spyOn(core, 'ScheduleRepository').mockImplementation(() => ({ getAssignmentScheduleForAgency }));
    const findExceptionsByVisitForAgency = vi.fn().mockResolvedValue(options.exceptions ?? []);
    vi.spyOn(core, 'EvvExceptionRepository').mockImplementation(() => ({ findExceptionsByVisitForAgency }));
    const findByVisitIdForAgency = vi.fn().mockResolvedValue(options.corrections ?? []);
    vi.spyOn(core, 'VisitMaintenanceRepository').mockImplementation(() => ({ findByVisitIdForAgency }));
    const findByEntityForAgency = vi.fn(async (aId, entityType, entityId) => options.auditEventsByEntity?.(aId, entityType, entityId) ?? []);
    const auditCreate = vi.fn(options.auditCreate ?? (async () => ({})));
    vi.spyOn(core, 'AuditEventRepository').mockImplementation(() => ({ findByEntityForAgency, create: auditCreate }));
...
describe('GET /admin/audit-packet/:visitId', () => {
    describe('capability gating', () => {
        it.each(['coordinator', 'caregiver', 'family'])('rejects %s with 403', async (role) => {
            installMocks();
    ...
    it('assembles the full §3.3 response shape', async () => {
        ...
        expect(Object.keys(body.curesActElements).sort()).toEqual([...core.curesActEvvDataPoints].sort());
    ...
    describe('geofence derivation', () => {
        it('reports "within" with distanceM <= allowedM for an in-fence visit', async () => {
            ...
            expect(response.body.geofence.clockIn.distanceM).toBeLessThanOrEqual(response.body.geofence.clockIn.allowedM);
        });
...
//# sourceMappingURL=audit-packet-routes.test.js.map