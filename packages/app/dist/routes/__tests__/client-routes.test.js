...
describe('client routes', () => {
    ...
    it('creates a client and persists the EVV geofence anchor', async () => {
        const createClient = vi.fn().mockImplementation((_agencyId, client) => Promise.resolve({ id: 'client-1', ...client }));
        vi.spyOn(core, 'ClientRepository').mockImplementation(() => ({ createClient }));
    ...
    it('updates a client (partial) and returns the result', async () => {
        ...
        vi.spyOn(core, 'ClientRepository').mockImplementation(() => ({ updateClient }));
        const response = await request(createApp())
...
//# sourceMappingURL=client-routes.test.js.map