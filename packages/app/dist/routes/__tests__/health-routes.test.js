...
function makeFakeDb(opts = {}) {
    const tableFn = (_name) => ({
        max: (_cols) => ({
            first: () => opts.auditMaxImpl
                ? opts.auditMaxImpl()
                : Promise.resolve({ lastEventAt: null }),
        }),
    });
    const db = Object.assign(tableFn, {
        raw: (sql) => opts.rawImpl ? opts.rawImpl(sql) : Promise.resolve({ rows: [{ '?column?': 1 }] }),
    });
    return db;
}
function makeApp(db) {
...
describe('GET /health/audit', () => {
    it('returns status="ok" when last event is within the last 24h', async () => {
        ...
        const app = makeApp(makeFakeDb({ auditMaxImpl: () => Promise.resolve({ lastEventAt: oneHourAgo }) }));
    ...
    it('returns status="stale" when last event is older than 24h', async () => {
        ...
        const app = makeApp(makeFakeDb({ auditMaxImpl: () => Promise.resolve({ lastEventAt: twoDaysAgo }) }));
        const res = await request(app).get('/health/audit');
        expect(res.status).toBe(200);
    ...
    it('returns status="empty" when audit_events has no rows', async () => {
        const app = makeApp(makeFakeDb({ auditMaxImpl: () => Promise.resolve({ lastEventAt: null }) }));
        const res = await request(app).get('/health/audit');
        expect(res.status).toBe(200);
