...
function stubLookups(opts) {
    ...
    vi.spyOn(core, 'AgencyRepository').mockImplementation(function () {
        return {
...
describe('POST /invites', () => {
    ...
    it('returns emailDelivery: sent when SES accepts the message', async () => {
        ...
        const sesSpy = vi
            ...
            .mockResolvedValue({ MessageId: 'msg-abc-123' });
        const response = await request(createApp())
            .post('/invites')
            .set('Authorization', `Bearer ${makeToken('admin')}`)
            .send({ email: 'caregiver@keystone.example', role: 'caregiver' });
        ...
        const sentCalls = auditCreate.mock.calls.filter((c) => c[0].eventType === 'invite.email.sent');
        expect(sentCalls).toHaveLength(1);
        const payload = (sentCalls[0]?.[0]).payload;
    ...
    it('returns emailDelivery: failed when SES rejects the request', async () => {
        ...
        const failedCalls = auditCreate.mock.calls.filter((c) => c[0].eventType === 'invite.email.failed');
        expect(failedCalls).toHaveLength(1);
        const payload = (failedCalls[0]?.[0]).payload;
...
describe('POST /invites/:id/resend-email', () => {
    it('resends the email for a pending invite and returns emailDelivery: sent', async () => {
        ...
        vi.spyOn(SESv2Client.prototype, 'send')
            .mockResolvedValue({ MessageId: 'msg-resend-1' });
        const response = await request(createApp())
