...
describe('PUT /agencies/me/hhaexchange-config', () => {
    ...
    it('allows enabled=true when identity fields are present', async () => {
        ...
        const response = await request(app)
            ...
            .send({ enabled: true });
        expect(response.status).toBe(200);
        expect(upsert).toHaveBeenCalledWith(expect.objectContaining({ enabled: true, agencyTaxId: '123456789' }));
    });
...
//# sourceMappingURL=agency-hhaexchange-config-routes.test.js.map