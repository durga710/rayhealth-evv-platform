...
const CLIENTS_CSV = 'external_id,first_name,last_name,date_of_birth\n' +
...
describe('import routes', () => {
    ...
    it('refuses a commit with validation errors (422, nothing written)', async () => {
        ...
        vi.spyOn(core, 'ClientRepository').mockImplementation(() => ({ upsertClientForImport }));
        const res = await request(createApp())
            .post('/import/clients/commit')
            .set('Authorization', `Bearer ${makeToken('admin')}`)
            .set('content-type', 'text/csv')
...
//# sourceMappingURL=import-routes.test.js.map