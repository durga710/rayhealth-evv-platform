...
function makeMockDb(state) {
    const buildBuilder = (table) => {
        const ctx = { table, filters: [], insertRow: null };
        const builder = {
            where(filter) {
            ...
            async first(col) {
                if (ctx.table === 'staff_invites') {
                    ...
                    if (filter.token === state.invite.token)
                        return state.invite;
                    return undefined;
                }
                if (ctx.table === 'agencies' && col === 'name')
                    return { name: 'Keystone Home Care' };
                if (ctx.table === 'users') {
                    const filter = Object.assign({}, ...ctx.filters);
            ...
            async update(data) {
                if (ctx.table === 'staff_invites' && data.status === 'accepted') {
                    ...
                    state.invite.accepted_at = data.accepted_at ?? new Date();
                    return 1;
                }
                return 1;
            },
    ...
    const fn = ((table) => buildBuilder(table));
    fn.raw = (s) => s;
    fn.fn = { now: () => 'NOW()' };
...
describe('invite acceptance routes', () => {
    ...
    describe('POST /invites/accept/:token', () => {
        it('creates caregiver + user, marks invite accepted, returns bearer token', async () => {
            ...
            const matches = await bcrypt.compare('CorrectHorseStaple12!', state.insertedUser?.password_hash);
            expect(matches).toBe(true);
            expect(state.inviteUpdated).toBe(true);
            expect(audit).toHaveBeenCalledWith(expect.objectContaining({ eventType: 'invite.accepted', outcome: 'success' }));
        });
...
//# sourceMappingURL=invite-acceptance-routes.test.js.map