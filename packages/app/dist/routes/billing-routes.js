...
router.post('/checkout-session', authContext, async (req, res) => {
    ...
    if (!tier || !(tier in TIER_PRICE_IDS) || !TIER_PRICE_IDS[tier]) {
    ...
    try {
        ...
        const session = await sdk.checkout.sessions.create({
            ...
            metadata: { agencyId: req.auth.agencyId, tier: tier },
            subscription_data: { metadata: { agencyId: req.auth.agencyId, tier: tier } },
        });
...
router.post('/webhook', async (req, res) => {
    ...
    try {
        if (event.type === 'checkout.session.completed') {
            ...
            if (agencyId)
                await repo.updateBilling(agencyId, { subscriptionStatus: sub.status });
        }
