...
router.patch('/notifications', async (req, res) => {
    ...
    try {
        const db = req.app.get('db');
        const existing = (await db('users').where({ id: req.auth.userId }).select('notification_prefs').first());
...
router.patch('/preferences', async (req, res) => {
    ...
    try {
        const db = req.app.get('db');
        const existing = (await db('users').where({ id: req.auth.userId }).select('preferences').first());
...
router.post('/2fa/setup', async (req, res) => {
    try {
        const db = req.app.get('db');
        const row = (await db('users').where({ id: req.auth.userId }).select('email', 'totp_enabled').first());
        if (!row) {
            res.status(404).json({ message: 'User not found' });
            return;
        }
...
router.post('/2fa/enable', async (req, res) => {
    ...
    try {
        const db = req.app.get('db');
        const row = (await db('users').where({ id: req.auth.userId }).select('totp_secret', 'totp_enabled').first());
        if (!row?.totp_secret) {
...
router.post('/2fa/disable', async (req, res) => {
    ...
    try {
        const db = req.app.get('db');
        const row = (await db('users').where({ id: req.auth.userId }).select('password_hash').first());
        if (!row) {
            res.status(404).json({ message: 'User not found' });
            return;
        }
...
router.get('/export', async (req, res) => {
    try {
        ...
        let training = [];
        const caregiverId = user.caregiver_id ?? null;
        if (caregiverId) {
