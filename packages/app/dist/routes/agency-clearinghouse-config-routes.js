...
import { z } from 'zod';
import { AgencyClearinghouseConfigRepository, AuditEventRepository, } from '@rayhealth/core';
...
function validateForEnable(next) {
    if (!next.enabled)
        return null;
    if (!next.endpoint)
        return 'cannot enable: endpoint is required';
    return null;
}
...
router.get('/me/clearinghouse-config', requireCapability('agency.read'), async (req, res) => {
    try {
...
router.put('/me/clearinghouse-config', requireCapability('agency.write'), async (req, res) => {
    try {
        ...
        try {
            ...
            process.stderr.write(`[audit-write-failed] agency.evv-config.changed (clearinghouse) agency=${req.auth.agencyId} ` +
...
//# sourceMappingURL=agency-clearinghouse-config-routes.js.map