...
const DB_QUERY_TIMEOUT_MS = 2000;
...
router.get('/audit', async (req, res) => {
    ...
    try {
        const db = req.app.get('db');
        const row = await withTimeout(db('audit_events').max({ lastEventAt: 'occurred_at' }).first(), DB_QUERY_TIMEOUT_MS, 'health/audit');
        const lastEventAtRaw = row?.lastEventAt ?? null;
        const lastEventAt = lastEventAtRaw ? new Date(lastEventAtRaw).toISOString() : null;
        const ageSeconds = lastEventAt === null
            ? null
            : Math.max(0, Math.floor((Date.now() - new Date(lastEventAt).getTime()) / 1000));
        let status;
        if (lastEventAt === null)
            status = 'empty';
        else if (ageSeconds !== null && ageSeconds > STALE_THRESHOLD_SECONDS)
            status = 'stale';
        else
            status = 'ok';
        res.status(200).json({
