...
import { ComplianceEngineRepository, LearningRepository, RecurringScheduleRepository, buildCommandCenterAttention, buildBriefingPrompt, deriveTodayVisitStatus, } from '@rayhealth/core';
 ...
 */
async function composeCommandCenter(db, agencyId) {
    ...
    const coverageEnd = new Date(Date.now() + 14 * 86400000).toISOString().slice(0, 10);
    ...
    const [today, exceptions, authorizations, credentials, claims, payroll, rollup, forecast] = await Promise.all([
