...
router.post('/courses', requireCapability('staff.write'), async (req, res) => {
...
router.patch('/courses/:id', requireCapability('staff.write'), async (req, res) => {
...
router.delete('/courses/:id', requireCapability('staff.write'), async (req, res) => {
    try {
        const db = req.app.get('db');
        const repo = new LearningRepository(db);
...
router.get('/certificate/:courseId', requireCapability('evv.write'), async (req, res) => {
    try {
        const db = req.app.get('db');
        const repo = new LearningRepository(db);
        const caregiverId = req.auth.caregiverId ?? req.auth.userId;
