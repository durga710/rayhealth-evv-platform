...
const PA_SERVICE_CODES = ['T1019', 'S5125', 'T1004', 'T1021'];
const feeScheduleSchema = z.record(z.enum(PA_SERVICE_CODES), z.number().int().min(0).max(1000000));
...
router.put('/current/billing', requireCapability('agency.write'), async (req, res) => {
    ...
    try {
        const db = req.app.get('db');
        const updated = await new AgencyRepository(db).updateBillingIdentity(req.auth.agencyId, parsed.data);
        if (!updated) {
            res.status(404).json({ message: 'Agency not found' });
            return;
        }
