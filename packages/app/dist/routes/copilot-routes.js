...
import { AuditEventRepository, copilotActionSchema, parseAgencyFeatures, } from '@rayhealth/core';
import { GeminiApiError, GeminiNotConfiguredError, askGemini, isGeminiConfigured } from '../services/gemini-client.js';
import { ActionAuthorizationError, ActionExecutionError, executeCopilotAction, } from '../services/copilot-action-executor.js';
import { buildCopilotContext, contextSizeSummary } from '../services/copilot-context.js';
const router = Router();
async function readFeatures(req) {
    const db = req.app.get('db');
    if (!db)
        return parseAgencyFeatures(undefined);
    const row = (await db('agencies').where({ id: req.auth.agencyId }).first('features'));
...
router.post('/ask', async (req, res) => {
    ...
    try {
        ...
        const model = body.model ?? (features.aiCopilot.plan === 'pro' ? 'gemini-2.5-pro' : 'gemini-2.5-flash');
        ...
        if (dataMatch)
            answerText = answerText.replace(dataMatch[0], '').trim();
        ...
        try {
            ...
            process.stderr.write(`[audit-write-failed] copilot.query err=${auditErr instanceof Error ? auditErr.message : 'unknown'}\n`);
        }
        res.json({
            success: true,
            data: {
...
router.post('/execute', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            process.stderr.write(`[audit-write-failed] copilot.action.confirmed err=${auditErr instanceof Error ? auditErr.message : 'unknown'}\n`);
        }
