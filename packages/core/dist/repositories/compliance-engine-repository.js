...
export class ComplianceEngineRepository {
    ...
    async getAuditDefensePreview(agencyId, fromIso, toIso) {
     ...
     */
    async buildAuditDefensePacket(agencyId, fromIso, toIso) {
        const counts = await this.getAuditDefensePreview(agencyId, fromIso, toIso);
        const auditRows = await this.db('audit_events')
            .where('agency_id', agencyId)
            .whereBetween('occurred_at', [fromIso, toIso])
            .select('id', 'occurred_at', 'actor_id', 'actor_type', 'event_type', 'entity_type', 'entity_id', 'outcome', 'correlation_id', 'payload');
        const vmurRows = await this.db('visit_maintenance')
            .where('agency_id', agencyId)
            .whereBetween('created_at', [fromIso, toIso])
            .select('id', 'visit_id', 'requester_id', 'reason', 'status', 'original_start_time', 'original_end_time', 'adjusted_start_time', 'adjusted_end_time', 'created_at');
        const visitRows = await this.db('evv_visits')
            .join('caregivers', 'evv_visits.caregiver_id', 'caregivers.id')
            .where('caregivers.agency_id', agencyId)
            .whereBetween('evv_visits.clock_in_time', [fromIso, toIso])
            .select('evv_visits.id as id', 'evv_visits.assignment_id as assignment_id', 'evv_visits.caregiver_id as caregiver_id', 'evv_visits.clock_in_time as clock_in_time', 'evv_visits.clock_out_time as clock_out_time', 'evv_visits.status as status', 'evv_visits.created_at as created_at');
     ...
     */
    async listOpenExceptions(agencyId, opts = {}) {
        const limit = clamp(opts.limit ?? 50, 1, 200);
        const offset = Math.max(0, opts.offset ?? 0);
        const baseScope = () => this.db('evv_exceptions')
        ...
        const [totalRow] = await filtered().count('* as count');
        ...
        const rows = await filtered()
            ...
            .limit(limit)
            .offset(offset)
            .select('evv_exceptions.id as id', 'evv_exceptions.visit_id as visit_id', 'evv_exceptions.exception_type as exception_type', 'evv_exceptions.reason as reason', 'evv_exceptions.created_at as created_at', 'evv_visits.caregiver_id as caregiver_id', 'evv_visits.clock_in_time as visit_clock_in_time', 'evv_visits.status as visit_status', 'caregivers.agency_id as agency_id');
        const toIsoString = (value) => {
            if (value instanceof Date)
                return value.toISOString();
            if (typeof value === 'string') {
     ...
     */
    async acknowledgeException(agencyId, exceptionId, actorId) {
        return this.db.transaction(async (trx) => {
            const exception = await trx('evv_exceptions')
                ...
                .first('evv_exceptions.id as id', 'evv_exceptions.visit_id as visit_id', 'evv_exceptions.exception_type as exception_type', 'evv_exceptions.reason as reason', 'evv_exceptions.approved_at as approved_at');
            if (!exception)
                return null;
     ...
     * cycle.
     */
    async listAuthorizations(agencyId, opts) {
        const asOfDate = new Date(`${opts.asOf}T00:00:00.000Z`);
        const fmt = (d) => d.toISOString().slice(0, 10);
        const asOfStr = fmt(asOfDate);
        const horizon14 = fmt(new Date(asOfDate.getTime() + 14 * 86400000));
        const horizon30 = fmt(new Date(asOfDate.getTime() + 30 * 86400000));
        const horizon90 = fmt(new Date(asOfDate.getTime() + 90 * 86400000));
        const recentExpiredStart = fmt(new Date(asOfDate.getTime() - 14 * 86400000));
        const limit = clamp(opts.limit ?? 25, 1, 200);
        const offset = Math.max(0, opts.offset ?? 0);
        const filter = opts.filter ?? 'active';
        const scoped = () => this.db('authorizations')
            .join('clients', 'authorizations.client_id', 'clients.id')
            .where('clients.agency_id', agencyId);
        const applyFilter = (qb) => {
            switch (filter) {
                case 'active':
                    return qb
        ...
        const [totalRow] = await applyFilter(scoped()).count('* as count');
        const total = Number(totalRow?.count ?? 0);
        const baseRows = (await applyFilter(scoped())
            ...
            .limit(limit)
            .offset(offset)
            .select('authorizations.id as id', 'authorizations.client_id as client_id', 'authorizations.payer_id as payer_id', 'authorizations.service_code as service_code', 'authorizations.units_authorized as units_authorized', 'authorizations.start_date as start_date', 'authorizations.end_date as end_date', 'clients.first_name as first_name', 'clients.last_name as last_name'));
        ...
        const usedByAuth = new Map();
        if (idList.length > 0) {
            const usageRows = await this.db('authorizations')
                .join('clients', 'authorizations.client_id', 'clients.id')
                ...
                .andWhereRaw('evv_visits.clock_in_time::date BETWEEN authorizations.start_date AND authorizations.end_date')
                .groupBy('authorizations.id')
                .select('authorizations.id as auth_id', this.db.raw('COALESCE(SUM(EXTRACT(EPOCH FROM (evv_visits.clock_out_time - evv_visits.clock_in_time)) / 3600.0), 0)::float as hours'));
        ...
        const rows = baseRows.map((row) => {
            ...
            const daysToExpiry = Math.round((endMs - asOfMs) / 86400000);
     ...
     */
    async getAuthorizationOversight(agencyId, asOf) {
        const asOfDate = new Date(`${asOf}T00:00:00.000Z`);
        const fmt = (d) => d.toISOString().slice(0, 10);
        const asOfStr = fmt(asOfDate);
        const expiring14End = fmt(new Date(asOfDate.getTime() + 14 * 86400000));
        const expiring30End = fmt(new Date(asOfDate.getTime() + 30 * 86400000));
        const recentlyExpiredStart = fmt(new Date(asOfDate.getTime() - 14 * 86400000));
        const baseScope = () => this.db('authorizations')
            .join('clients', 'authorizations.client_id', 'clients.id')
            .where('clients.agency_id', agencyId);
     ...
     */
    async getEngineSummary(agencyId, asOf) {
        const asOfDate = new Date(`${asOf}T00:00:00.000Z`);
        const thirtyDaysAgo = new Date(asOfDate.getTime() - 30 * 86400000)
            .toISOString()
            .slice(0, 10);
        const [audit, authorizationOversight, exceptionResolution, medicaid, payroll, claims, credentials] = await Promise.all([
            this.getAuditDefensePreview(agencyId, `${thirtyDaysAgo}T00:00:00.000Z`, `${asOf}T23:59:59.999Z`),
     ...
     */
    async getCredentialsCompliance(agencyId, asOf) {
        const asOfDate = new Date(`${asOf}T00:00:00.000Z`);
        const fmt = (d) => d.toISOString().slice(0, 10);
        const asOfStr = fmt(asOfDate);
        const horizon30 = fmt(new Date(asOfDate.getTime() + 30 * 86400000));
        const horizon90 = fmt(new Date(asOfDate.getTime() + 90 * 86400000));
        const recentExpiredStart = fmt(new Date(asOfDate.getTime() - 14 * 86400000));
        const agencyScope = () => this.db('caregiver_credentials')
     ...
     */
    async getClaimMatching(agencyId) {
        const sevenDaysAgo = new Date(Date.now() - 7 * 86400000).toISOString();
        const thirtyDaysAgo = new Date(Date.now() - 30 * 86400000).toISOString();
        const agencyScope = () => this.db('evv_visits')
            .join('caregivers', 'evv_visits.caregiver_id', 'caregivers.id')
            .where('caregivers.agency_id', agencyId);
     ...
     */
    async getClaimReadinessBlockers(agencyId, limit = 100) {
        ...
        const rows = await this.db('evv_visits as v')
            ...
            .limit(limit + 1)
            .select('v.id as visit_id', 'v.status as status', 'v.clock_in_time', 'v.clock_out_time', 'cg.first_name as cg_first', 'cg.last_name as cg_last', 'c.first_name as client_first', 'c.last_name as client_last');
        const truncated = rows.length > limit;
        const page = truncated ? rows.slice(0, limit) : rows;
        const blockers = page.map((r) => {
            const reason = r.clock_in_time && !r.clock_out_time
                ? 'open'
                : r.status === 'flagged'
                    ? 'flagged'
                    : 'pending';
     ...
     * in-progress visits.
     */
    async getPayrollReconciliation(agencyId) {
        const sevenDaysAgo = new Date(Date.now() - 7 * 86400000).toISOString();
        const thirtyDaysAgo = new Date(Date.now() - 30 * 86400000).toISOString();
        const agencyScope = () => this.db('evv_visits')
            .join('caregivers', 'evv_visits.caregiver_id', 'caregivers.id')
        ...
        const row7 = await agencyScope()
            .where('evv_visits.clock_in_time', '>=', sevenDaysAgo)
            .whereNotNull('evv_visits.clock_out_time')
            .first(this.db.raw('COALESCE(SUM(EXTRACT(EPOCH FROM (evv_visits.clock_out_time - evv_visits.clock_in_time)) / 3600.0), 0)::float as hours'));
     ...
     */
    async getMedicaidWorkflow(agencyId, asOf) {
        const asOfDate = new Date(`${asOf}T00:00:00.000Z`);
        const fmt = (d) => d.toISOString().slice(0, 10);
        const asOfStr = fmt(asOfDate);
        const thirtyDaysAgo = fmt(new Date(asOfDate.getTime() - 30 * 86400000));
        const activeScope = () => this.db('authorizations')
            .join('clients', 'authorizations.client_id', 'clients.id')
            .where('clients.agency_id', agencyId)
            .where('authorizations.start_date', '<=', asOfStr)
            .where('authorizations.end_date', '>=', asOfStr);
        const [casesRow] = await activeScope()
            .countDistinct('authorizations.client_id as count');
        const [payersRow] = await activeScope()
            .countDistinct('authorizations.payer_id as count');
        const [serviceCodesRow] = await activeScope()
            .countDistinct('authorizations.service_code as count');
     ...
     */
    async getExceptionResolution(agencyId) {
        const openScope = () => this.db('evv_exceptions')
            .join('evv_visits', 'evv_exceptions.visit_id', 'evv_visits.id')
            .join('caregivers', 'evv_visits.caregiver_id', 'caregivers.id')
            .where('caregivers.agency_id', agencyId)
            .whereNull('evv_exceptions.approved_at');
        const [openRow] = await openScope().count('* as count');
     ...
     */
    async getTodaysVisitOps(agencyId, nowIso) {
        ...
        const rows = await this.db('assignments as a')
            ...
            .select('a.id as assignment_id', 'a.scheduled_start_time', 'v.clock_in_time', 'v.clock_out_time');
        let completed = 0;
        let inProgress = 0;
        let lateStart = 0;
        let upcoming = 0;
        for (const r of rows) {
            if (r.clock_out_time) {
                ...
                if (start !== null && now.getTime() > start + graceMs)
                    lateStart++;
                else
                    upcoming++;
            }
     ...
     */
    async getTodaysVisitBoard(agencyId) {
        const rows = await this.db('assignments as a')
            .join('visit_templates as vt', 'vt.id', 'a.visit_template_id')
            .join('clients as c', 'c.id', 'vt.client_id')
            ...
            .select('a.id as assignment_id', 'a.scheduled_start_time', 'c.first_name as client_first_name', 'c.last_name as client_last_name', 'cg.first_name as caregiver_first_name', 'cg.last_name as caregiver_last_name', 'v.clock_in_time', 'v.clock_out_time');
        return rows.map((r) => ({
 ...
 */
export function auditPacketRowToCsv(row) {
    return [
        row.recordType,
        row.id,
        row.occurredAt,
 ...
 */
function computePacketHash(rows) {
...
//# sourceMappingURL=compliance-engine-repository.js.map