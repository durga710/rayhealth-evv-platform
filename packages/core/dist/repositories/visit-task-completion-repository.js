...
export function normalizeVisitTaskPlan(value) {
...
export class VisitTaskCompletionRepository {
    ...
    async getForVisit(visitId, agencyId) {
        ...
        const rows = await this.db('visit_task_completions')
    ...
    async upsertBatch(input) {
        return this.db.transaction(async (trx) => {
            ...
            for (const completion of input.completions) {
                const replay = await trx('visit_task_completions')
                    .where({
                ...
                const existing = await trx('visit_task_completions')
                    .where({
                    agency_id: input.agencyId,
                    visit_id: input.visitId,
                ...
                let row;
                if (existing) {
                    [row] = await trx('visit_task_completions')
...
//# sourceMappingURL=visit-task-completion-repository.js.map