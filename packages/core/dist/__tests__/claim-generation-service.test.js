...
import { computeBillingUnits, generateClaims, minutesBetween, serviceDateOf, } from '../services/claim-generation-service.js';
...
function input(overrides = {}) {
    return {
        ...
        ratesByServiceCode: overrides.ratesByServiceCode ?? { T1019: 600, S5125: 600, T1004: 600, T1021: 5000 },
...
describe('generateClaims', () => {
    ...
    it('reports a visit with no matching authorization as unbillable', () => {
        const { claims, unbillable } = generateClaims(input({ visits: [visit({ clockInTime: '2026-07-10T14:00:00Z', clockOutTime: '2026-07-10T15:00:00Z' })] }));
    ...
    it('flags units exceeding remaining authorized units', () => {
        const { claims } = generateClaims(input({
    ...
    it('flags a missing client Medicaid id and missing NPI', () => {
        const { claims } = generateClaims(input({ visits: [visit({ clientMedicaidNumber: null, caregiverNpi: null })] }));
    ...
    it('reports a visit with no clock-out as unbillable', () => {
        const { claims, unbillable } = generateClaims(input({ visits: [visit({ clockOutTime: null })] }));
        expect(claims).toHaveLength(0);
        expect(unbillable[0].reasons[0]).toMatch(/no clock-out/);
    });
});
//# sourceMappingURL=claim-generation-service.test.js.map