...
import { parseCsv, validateImportRecords, parseAndValidate, } from '../services/import-service.js';
describe('parseCsv', () => {
...
describe('validateImportRecords — clients', () => {
    it('accepts a complete row and normalizes types', () => {
        const { records } = parseCsv('external_id,first_name,last_name,date_of_birth,medicaid_number,state,latitude,longitude,geofence_radius_m\n' +
...
describe('validateImportRecords — authorizations', () => {
    it('accepts a canonical service code and links via client_external_id', () => {
        const { records } = parseCsv('external_id,client_external_id,payer_id,service_code,units_authorized,start_date,end_date\n' +
    ...
    it('rejects a non-canonical (W-series) service code and end<start', () => {
        const { results } = parseAndValidate('authorizations', 'client_external_id,payer_id,service_code,units_authorized,start_date,end_date\n' +
    ...
    it('requires client_external_id for linking', () => {
        const { results } = parseAndValidate('authorizations', 'payer_id,service_code,units_authorized,start_date,end_date\nPA-MA,T1019,5,2026-06-01,2026-06-30\n');
