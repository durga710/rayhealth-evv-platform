...
import { mapClient, mapEmployee, mapVisit, toCCYYMMDD, validateClient, validateEmployee, validateVisit, } from '../integrations/sandata/index.js';
...
describe('validateClient', () => {
    ...
    it('hard-blocks a missing Medicaid id and address', () => {
        ...
        expect(res.issues.map((i) => i.field)).toEqual(expect.arrayContaining(['ClientMedicaidID', 'ClientAddressLine1']));
    });
...
describe('validateVisit', () => {
    ...
    it('soft-warns missing GPS but stays ok when GPS is not required', () => {
        ...
        const res = validateVisit(noGps);
        expect(res.ok).toBe(true);
        expect(res.issues.some((i) => i.severity === 'SOFT_WARN')).toBe(true);
    });
});
//# sourceMappingURL=sandata-alt-evv.test.js.map