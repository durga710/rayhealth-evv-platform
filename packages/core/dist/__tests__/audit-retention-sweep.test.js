...
async function clearAuditTables() {
...
async function insertAuditEvent(occurredAt) {
    ...
    const [row] = (await db('audit_events')
        ...
        .returning(['id', 'agency_id', 'event_type', 'occurred_at']));
    return row;
}
function yearsAgo(n) {
    const d = new Date();
...
describe('runAuditRetentionSweep', () => {
    ...
    it('archives rows older than the 6-year floor and leaves recent rows in hot', async () => {
        ...
        expect(inHot.map((r) => r.id)).toEqual([recent.id]);
        const inArchive = await db('audit_events_archive')
            .where({ agency_id: TEST_AGENCY_ID })
            .orderBy('occurred_at', 'asc');
        const archivedIds = inArchive.map((r) => r.id).sort();
...
//# sourceMappingURL=audit-retention-sweep.test.js.map