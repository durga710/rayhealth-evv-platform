...
import { toHhaexchangeCsv } from '../services/hhaexchange-mapping.js';
...
describe('EvvComplianceService — coordinate 0 handling (finding #15)', () => {
    it('does NOT treat lat/lng of exactly 0 as a missing location', () => {
        const result = new EvvComplianceService().validate(visit({ clockInLocation: { lat: 0, lng: 0, accuracy: 10 } }));
        expect(result.missingElements).not.toContain('location');
    });
    it('still flags a genuinely absent location', () => {
        const result = new EvvComplianceService().validate(visit({ clockInLocation: undefined }));
...
//# sourceMappingURL=evv-compliance-and-csv.test.js.map