...
import { buildSandataApiPayload, submitVisits, } from '../integrations/sandata-client.js';
...
describe('sandata payload mapping', () => {
    it('maps a mapped visit and skips unmapped caregiver/service', () => {
        ...
        const unmappedWorker = { ...visit, visitId: 'v-2', caregiverId: 'cg-x' };
        const unmappedService = { ...visit, visitId: 'v-3', serviceCode: 'XYZ' };
...
describe('sandata submission', () => {
    it('returns ok with per-visit acks on a 200 response', async () => {
        vi.stubGlobal('fetch', fetchReturning(200, { batchId: 'B-1', results: [{ visitOtherId: 'v-1', status: 'accepted', confirmationId: 'C-1' }] }));
    ...
    it('defaults sent visits to submitted when the response has no per-visit results', async () => {
        ...
        if (res.kind === 'ok')
            expect(res.acks[0]).toMatchObject({ visitId: 'v-1', status: 'submitted' });
    });
...
//# sourceMappingURL=sandata-client.test.js.map