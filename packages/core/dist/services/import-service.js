...
function validateClientRow(rec) {
    const errors = [];
    ...
    if (Number.isNaN(geo))
        errors.push('geofence_radius_m must be a number');
    else if (geo !== undefined && geo <= 0)
        errors.push('geofence_radius_m must be positive');
    if (errors.length > 0)
        return { errors };
    return {
        errors: [],
...
function validateCaregiverRow(rec) {
    const errors = [];
    const firstName = (rec.first_name ?? '').trim();
    const lastName = (rec.last_name ?? '').trim();
...
function validateAuthorizationRow(rec) {
    const errors = [];
    ...
    if (!clientExternalId)
        errors.push('client_external_id is required (links to a client external_id)');
    if (!payerId)
        errors.push('payer_id is required');
    if (!serviceCode)
        errors.push('service_code is required');
    else if (!paServiceCodes.includes(serviceCode)) {
        errors.push(`service_code must be one of: ${paServiceCodes.join(', ')}`);
    }
    const units = optNum(rec.units_authorized);
    if (units === undefined)
        errors.push('units_authorized is required');
    else if (Number.isNaN(units) || units <= 0)
        errors.push('units_authorized must be a positive number');
    if (!start)
 ...
 */
export function validateImportRecords(entity, records) {
    const validate = VALIDATORS[entity];
    return records.map((rec, idx) => {
        const { value, errors } = validate(rec);
        return errors.length > 0
            ? { rowNumber: idx + 1, status: 'error', errors }
            : { rowNumber: idx + 1, status: 'ok', value, errors: [] };
    });
}
/** Convenience: parse CSV text and validate in one call. */
export function parseAndValidate(entity, csv) {
