 ...
 */
export function deriveTodayVisitStatus(v, nowMs, graceMs = 15 * 60 * 1000) {
    if (v.clockOutTime)
        return 'completed';
    if (v.clockInTime)
        return 'in_progress';
    const start = v.scheduledStartTime ? new Date(v.scheduledStartTime).getTime() : null;
    if (start !== null && nowMs > start + graceMs)
        return 'late';
    return 'upcoming';
}
 ...
 */
export function buildCommandCenterAttention(s) {
    const items = [];
    const push = (item) => {
        if (item.count > 0)
            items.push(item);
    ...
    return items.sort((a, b) => SEVERITY_WEIGHT[a.severity] - SEVERITY_WEIGHT[b.severity] || b.count - a.count);
}
/**
 ...
 */
export function buildBriefingPrompt(s) {
    const attention = buildCommandCenterAttention(s);
    const system = 'You are the operations briefing assistant for a Pennsylvania home-care agency ' +
    ...
    const priorityList = attention.length > 0
        ? attention.map((a) => `- [${a.severity}] ${a.title}`).join('\n')
        : '- Nothing currently flagged.';
    const prompt = `Today's numbers:\n${lines.join('\n')}\n\n` +
