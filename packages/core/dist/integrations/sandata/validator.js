...
export function validateClient(client, ctx = {}) {
    const issues = [];
    const block = (field, message) => issues.push({ field, message, severity: 'HARD_BLOCK' });
    const warn = (field, message) => issues.push({ field, message, severity: 'SOFT_WARN' });
    if (!client.id)
        block('ClientCustomID', 'Client has no id to use as the Sandata custom id');
    if (!client.firstName?.trim())
        block('ClientFirstName', 'First name is required');
    if (!client.lastName?.trim())
        block('ClientLastName', 'Last name is required');
    if (!client.medicaidNumber?.trim())
        block('ClientMedicaidID', 'Medicaid ID is required for Sandata');
    if (!client.addressLine1?.trim()) {
        block('ClientAddressLine1', 'A service address is required');
    }
    else {
        if (!client.state || client.state.length !== 2)
            block('ClientState', 'State must be the 2-character code');
        if (!client.postalCode || !ZIP_RE.test(client.postalCode.replace(/-/g, ''))) {
            block('ClientZip', 'ZIP must be 5 or 9 digits');
        }
        if (!client.city?.trim())
            block('ClientCity', 'City is required');
    }
    const hasGps = client.latitude != null && client.longitude != null;
    if (!hasGps) {
        if (ctx.gpsRequired)
            block('ClientLatitude', 'Client GPS coordinates are required for this program');
        else
            warn('ClientLatitude', 'No client GPS anchor; visit geofencing cannot be verified');
    }
    return result(issues);
}
/** Validate a Caregiver for Sandata EMPLOYEE transmission. */
export function validateEmployee(caregiver) {
    const issues = [];
    const block = (field, message) => issues.push({ field, message, severity: 'HARD_BLOCK' });
    if (!caregiver.id)
        block('EmployeeCustomID', 'Caregiver has no id to use as the Sandata custom id');
    if (!caregiver.firstName?.trim())
        block('EmployeeFirstName', 'First name is required');
    if (!caregiver.lastName?.trim())
        block('EmployeeLastName', 'Last name is required');
    return result(issues);
}
...
export function validateVisit(visit, ctx = {}) {
    ...
    const warn = (field, message) => issues.push({ field, message, severity: 'SOFT_WARN' });
    if (!visit.id)
        block('VisitOtherID', 'Visit has no id to use as the Sandata visit id');
    if (!visit.clientId)
        block('ClientCustomID', 'Visit is not linked to a client');
    if (!visit.caregiverId)
        block('EmployeeCustomID', 'Visit is not linked to a caregiver');
    if (!visit.serviceCode)
        block('ProcedureCode', 'Visit has no service (procedure) code');
    ...
    if (!inGps || !outGps) {
        if (ctx.gpsRequired)
            block('CallLatitude', 'GPS is required on both clock-in and clock-out for this program');
        else
            warn('CallLatitude', 'Missing GPS on a call; Sandata will flag a location exception');
    }
    return result(issues);
}
