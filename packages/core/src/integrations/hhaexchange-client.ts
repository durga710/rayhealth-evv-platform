...
import type { AggregatorSubmitResult, IntegrationCredentials, VisitSubmission } from './types.js';

export interface HhaexchangeClientConfig {
  enabled: boolean;
...
export async function submitVisits(
  config: HhaexchangeClientConfig,
  _visits: VisitSubmission[],
): Promise<AggregatorSubmitResult> {
  if (!config.enabled) return { kind: 'not_configured', reason: 'HHAeXchange integration is disabled for this agency' };
  if (!config.apiBaseUrl) return { kind: 'not_configured', reason: 'No HHAeXchange API base URL configured' };
  ...
  if (!hasAuth) return { kind: 'not_configured', reason: 'No HHAeXchange API credentials configured' };
