...
} from '../services/command-center-service.js';

const clean: CommandCenterSnapshot = {
...
describe('buildCommandCenterAttention', () => {
  ...
  it('pluralizes titles correctly for a single item', () => {
    const s: CommandCenterSnapshot = { ...clean, today: { ...clean.today, lateStart: 1 } };
  ...
  it('surfaces ungenerated coverage gaps as a warning linking to recurring schedules', () => {
    const s: CommandCenterSnapshot = { ...clean, coverage: { totalGaps: 4 } };
    const items = buildCommandCenterAttention(s);
...
describe('deriveTodayVisitStatus', () => {
  ...
  it('is completed once clocked out (regardless of other fields)', () => {
    expect(
      deriveTodayVisitStatus(
        { clockInTime: '2026-06-28T09:00:00Z', clockOutTime: '2026-06-28T13:00:00Z', scheduledStartTime: '2026-06-28T09:00:00Z' },
        now,
      ),
    ).toBe('completed');
  });
  ...
  it('is in_progress when clocked in but not out', () => {
    expect(
      deriveTodayVisitStatus(
        { clockInTime: '2026-06-28T13:30:00Z', clockOutTime: null, scheduledStartTime: '2026-06-28T13:00:00Z' },
        now,
      ),
    ).toBe('in_progress');
  });
  ...
  it('is late when start + grace has passed with no clock-in', () => {
    ...
    expect(
      deriveTodayVisitStatus(
        { clockInTime: null, clockOutTime: null, scheduledStartTime: '2026-06-28T13:00:00Z' },
        now,
      ),
    ).toBe('late');
  });
  ...
  it('is upcoming when still within grace / scheduled later', () => {
    ...
    expect(
      deriveTodayVisitStatus(
        { clockInTime: null, clockOutTime: null, scheduledStartTime: '2026-06-28T13:50:00Z' },
        now,
      ),
    ).toBe('upcoming');
    ...
    expect(
      deriveTodayVisitStatus(
        { clockInTime: null, clockOutTime: null, scheduledStartTime: '2026-06-28T16:00:00Z' },
        now,
      ),
    ).toBe('upcoming');
  });
  ...
  it('treats a missing scheduled time as upcoming (cannot be late without a start)', () => {
    expect(
      deriveTodayVisitStatus({ clockInTime: null, clockOutTime: null, scheduledStartTime: null }, now),
    ).toBe('upcoming');
  });
});
