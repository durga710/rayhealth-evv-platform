...
function makeFakeDb(initial?: FakeRow) {
  ...
  const builder = {
    ...
    merge: async (data: Record<string, unknown>) => {
      if (row) {
        row = {
          ...row,
          agency_tax_id: (data.agency_tax_id as string | null) ?? row.agency_tax_id,
          hha_provider_id: (data.hha_provider_id as string | null) ?? row.hha_provider_id,
        ...
        row = {
          ...
          agency_tax_id: (data.agency_tax_id as string | null) ?? null,
          hha_provider_id: (data.hha_provider_id as string | null) ?? null,
  ...
  return { db, setRow(next: FakeRow | undefined) { row = next; }, getRow() { return row; } };
}
