...
import { SandataApiClient, SandataEntityType, type SandataAltEvvConfig } from '../integrations/sandata/index.js';

const config: SandataAltEvvConfig = {
...
describe('SandataApiClient.post', () => {
  it('sends Basic auth + EntityGuid, strips the trailing slash, and returns the uuid', async () => {
    ...
    if (out.kind === 'accepted') expect(out.uuid).toBe('batch-uuid-1');
...
describe('SandataApiClient.getStatus', () => {
  it('hits the status path and returns ready results', async () => {
    ...
    if (out.kind === 'ready') expect(out.response.records?.[0]?.externalID).toBe('c1');
