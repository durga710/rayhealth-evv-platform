...
import {
  SandataEntityType,
  SandataRecordStatus,
  SandataTransmissionService,
  type ExceptionInput,
  type PendingTransmission,
  type PostedRecord,
  type RecordStatusUpdate,
  type SandataApiClient,
  type SandataStateRepo,
...
class FakeState implements SandataStateRepo {
  seq = new Map<string, number>();
  status = new Map<string, SandataRecordStatus>();
  transmissions: Array<PendingTransmission & { status: string }> = [];
  exceptions: ExceptionInput[] = [];
  polled: number[] = [];
  applied: Array<{ id: number; results: RecordStatusUpdate[] }> = [];
  private nextId = 1;

  private key(entity: SandataEntityType, ext: string) {
    return `${entity}|${ext}`;
  }

  async nextSequence(_agency: string, entity: SandataEntityType, ext: string) {
    return (this.seq.get(this.key(entity, ext)) ?? 0) + 1;
  }

  async recordTransmitted(
    agencyId: string,
    entity: SandataEntityType,
    uuid: string,
    _env: string,
  ...
  async applyStatusResults(id: number, results: RecordStatusUpdate[]) {
    const tx = this.transmissions.find((t) => t.id === id);
    if (!tx) return;
    for (const r of results) this.status.set(this.key(tx.entityType, r.externalId), r.status);
    tx.status = 'COMPLETED';
    this.applied.push({ id, results });
  }

  async markTransmissionPolled(id: number) {
    this.polled.push(id);
    const tx = this.transmissions.find((t) => t.id === id);
    if (tx) tx.pollAttempts += 1;
  }

  async areDependenciesVerified(_agency: string, clientExt: string, employeeExt: string) {
    return (
      this.status.get(this.key(SandataEntityType.CLIENT, clientExt)) === SandataRecordStatus.VERIFIED &&
      this.status.get(this.key(SandataEntityType.EMPLOYEE, employeeExt)) === SandataRecordStatus.VERIFIED
    );
  }

  async enqueueException(_agency: string, input: ExceptionInput) {
    this.exceptions.push(input);
  }

  /** Test helper: force a record to VERIFIED. */
  verify(entity: SandataEntityType, ext: string) {
    this.status.set(this.key(entity, ext), SandataRecordStatus.VERIFIED);
  }
}

function fakeApi(handlers: Partial<SandataApiClient>): SandataApiClient {
  return handlers as unknown as SandataApiClient;
}
