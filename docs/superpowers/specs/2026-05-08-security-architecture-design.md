...
- The web app stores bearer JWTs in `localStorage` in [packages/web/src/lib/AuthContext.tsx](packages/web/src/lib/AuthContext.tsx:1).
- The mobile app still uses a mock in-memory authentication context in [packages/mobile/src/lib/AuthContext.tsx](packages/mobile/src/lib/AuthContext.tsx:1).
- API audit logging is only a console log stub in [packages/app/src/middleware/audit-log.ts](packages/app/src/middleware/audit-log.ts:1).
